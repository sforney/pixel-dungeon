...
public class WndCatalogus extends WndTabbed {
	...
	private static final String TXT_POTIONS = Game
			.getVar(R.string.WndCatalogus_Potions);
	private static final String TXT_SCROLLS = Game
			.getVar(R.string.WndCatalogus_Scrolls);
	private static final String TXT_TITLE = Game
			.getVar(R.string.WndCatalogus_Title);
	...
	public WndCatalogus(PotionInfo potionInfo) {
		super();
	...
	private void updateList() {
		...
		float pos = 0;
		if (showPotions) {
			for(PotionType type : potionInfo.getAllKnown()) {
				ListItem item = new ListItem(type.getClazz());
			...
			for(PotionType type : potionInfo.getAllUnknown()) {
				ListItem item = new ListItem(type.getClazz());
				item.setRect(0, pos, width, ITEM_HEIGHT);
				content.add(item);
				items.add(item);
