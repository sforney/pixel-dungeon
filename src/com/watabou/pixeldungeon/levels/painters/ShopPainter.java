...
public class ShopPainter extends Painter {
	...
	private static Item[] range() {
		...
		case 6:
			items.add((Random.Int(2) == 0 ? new Quarterstaff() : new Spear())
					.identify());
		...
		case 11:
			items.add((Random.Int(2) == 0 ? new Sword() : new Mace())
					.identify());
		...
		case 16:
			items.add((Random.Int(2) == 0 ? new Longsword() : new BattleAxe())
					.identify());
		...
		items.add(new PotionOfHealing(Dungeon.potionInfo));
		for (int i = 0; i < 3; i++) {
